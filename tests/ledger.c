...
void test_asset_with_token_address(void) {
    ...
    assert(cma_ledger_retrieve_asset(ledger, &asset_id_find, &token_address1, NULL, CMA_LEDGER_ASSET_TYPE_TOKEN_ADDRESS,
    ...
    assert(memcmp(token_address_find.data, token_address1.data, CMT_ABI_ADDRESS_LENGTH) == 0);
...
void test_asset_with_token_address_and_id(void) {
    ...
    assert(memcmp(token_address_find.data, token_address1.data, CMT_ABI_ADDRESS_LENGTH) == 0);
    assert(memcmp(token_id_find.data, token_id1.data, CMT_ABI_ADDRESS_LENGTH) == 0);
...
void test_account_address(void) {
    ...
    assert(cma_ledger_retrieve_account(ledger, &account_id_found, &account1, NULL, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    ...
    assert(cma_ledger_retrieve_account(ledger, &account_id_to_find, &account_found, NULL, CMA_LEDGER_ACCOUNT_TYPE_ID,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(memcmp(account_found.address.data, account1.address.data, CMT_ABI_ADDRESS_LENGTH) == 0);
    ...
    account_id_found = 99;
    assert(cma_ledger_retrieve_account(ledger, &account_id_found, &account_to_find, NULL, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(account_id_found == 0);

    cma_ledger_account_t account_found2 = {};
    assert(cma_ledger_retrieve_account(ledger, &account_id_to_find, &account_found2, NULL, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_ERROR_ACCOUNT_NOT_FOUND);
    ...
    assert(cma_ledger_retrieve_account(ledger, &account_id_found, NULL, &address2, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    ...
    assert(cma_ledger_retrieve_account(ledger, NULL, &account_found2, &address2, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(memcmp(account_found2.address.data, account2.address.data, CMT_ABI_ADDRESS_LENGTH) == 0);

    assert(cma_ledger_retrieve_account(ledger, &account_id_found, &account_found2, &address2, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(account_id_found == 1);
    assert(memcmp(account_found2.address.data, account2.address.data, CMT_ABI_ADDRESS_LENGTH) == 0);
    ...
    assert(cma_ledger_retrieve_account(ledger, &account_id_created, &account_created, &address3, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_CREATE) == CMA_LEDGER_SUCCESS);
    assert(account_id_created == 2);
    assert(memcmp(account_created.address.data, address3.data, CMT_ABI_ADDRESS_LENGTH) == 0);
...
void test_account_full_id(void) {
    ...
    assert(cma_ledger_retrieve_account(ledger, &account_id_found, &account_to_find2, NULL, CMA_LEDGER_ACCOUNT_TYPE_WALLET_ADDRESS,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(account_id_found == 1);

    cma_ledger_account_t account_found2 = {};
    assert(cma_ledger_retrieve_account(ledger, &account_id_to_find, &account_found2, NULL, CMA_LEDGER_ACCOUNT_TYPE_ACCOUNT_ID,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_ERROR_ACCOUNT_NOT_FOUND);
    ...
    assert(cma_ledger_retrieve_account(ledger, &account_id_found, NULL, &full_account2, CMA_LEDGER_ACCOUNT_TYPE_ACCOUNT_ID,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(account_id_found == 1);

    assert(cma_ledger_retrieve_account(ledger, NULL, &account_found2, &full_account2, CMA_LEDGER_ACCOUNT_TYPE_ACCOUNT_ID,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(memcmp(account_found2.account_id.data, account2.account_id.data, CMT_ABI_ADDRESS_LENGTH) == 0);

    assert(cma_ledger_retrieve_account(ledger, &account_id_found, &account_found2, &full_account2, CMA_LEDGER_ACCOUNT_TYPE_ACCOUNT_ID,
               CMA_LEDGER_OP_FIND) == CMA_LEDGER_SUCCESS);
    assert(account_id_found == 1);
    assert(memcmp(account_found2.account_id.data, account2.account_id.data, CMT_ABI_ADDRESS_LENGTH) == 0);
    ...
    cma_ledger_account_id_t account_id_created;
    cma_ledger_account_t account_created;
    assert(cma_ledger_retrieve_account(ledger, &account_id_created, &account_created, &full_account3, CMA_LEDGER_ACCOUNT_TYPE_ACCOUNT_ID,
               CMA_LEDGER_OP_CREATE) == CMA_LEDGER_SUCCESS);
    assert(account_id_created == 2);
    assert(memcmp(account_created.account_id.data, full_account3.data, CMT_ABI_ADDRESS_LENGTH) == 0);
...
void test_asset_total_supply_balance(void) {
    ...
    assert(memcmp(out_total_supply.data, zero_amount.data, CMT_ABI_U256_LENGTH) == 0);

    cma_amount_t balance = {};
    assert(cma_ledger_get_balance(ledger, 1000, 1000, &balance) ==
        CMA_LEDGER_SUCCESS);
    assert(memcmp(balance.data, zero_amount.data, CMT_ABI_U256_LENGTH) == 0);
...
void test_deposit(void) {
    ...
    assert(cma_ledger_get_total_supply(ledger, asset_id, &supply) ==
        CMA_LEDGER_SUCCESS);
    assert(memcmp(supply.data, amount.data, CMT_ABI_U256_LENGTH) == 0);

    cma_amount_t balance = {};
    assert(cma_ledger_get_balance(ledger, asset_id, account_id, &balance) ==
        CMA_LEDGER_SUCCESS);
    assert(memcmp(balance.data, amount.data, CMT_ABI_U256_LENGTH) == 0);
    ...
    assert(cma_ledger_get_total_supply(ledger, asset_id, &supply) ==
        CMA_LEDGER_SUCCESS);
    assert(memcmp(supply.data, amount2.data, CMT_ABI_U256_LENGTH) == 0);

    assert(cma_ledger_get_balance(ledger, asset_id, account_id, &balance) ==
        CMA_LEDGER_SUCCESS);
    assert(memcmp(balance.data, amount2.data, CMT_ABI_U256_LENGTH) == 0);
    ...
    assert(cma_ledger_get_total_supply(ledger, asset_id, &supply) ==
        CMA_LEDGER_SUCCESS);
    assert(memcmp(supply.data, amount4.data, CMT_ABI_U256_LENGTH) == 0);
...
void test_transfer(void) {
    ...
    assert(cma_ledger_get_balance(ledger, asset_id, account_id2, &balance) ==
        CMA_LEDGER_SUCCESS);
    assert(memcmp(balance.data, amount3.data, CMT_ABI_U256_LENGTH) == 0);

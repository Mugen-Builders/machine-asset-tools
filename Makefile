...
libcma_OBJ       := $(patsubst %.c,$(libcma_OBJDIR)/%.o,$(libcma_SRC))
libcma_LIB       := $(libcma_OBJDIR)/libcma.a
libcma_SO        := $(libcma_OBJDIR)/libcma.so

$(libcma_OBJ): $(libcma_OBJDIR)/%.o: %.c
	@mkdir -p $(@D)
	$(CC) $(CFLAGS) $(LIBCMA_CFLAGS) -MT $@ -MMD -MP -MF $(@:.o=.d) -c -o $@ $<
...
$(test_OBJDIR)/ledger: tests/ledger.c $(libcma_LIB)
	$(CC) $(CFLAGS) -o $@ $^

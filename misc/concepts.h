...
void is_base_of_test(const Base<Ts...>&);
} // namespace detail

template <template <typename...> class Base, typename Derived>
concept is_template_base_of = requires(const Derived& d) {
    detail::is_base_of_test<Base>(d);
};

// Check if class is based on template
template <template <typename...> class Base, typename Derived>
constexpr bool is_template_base_of_v = is_template_base_of<Base, Derived>;
